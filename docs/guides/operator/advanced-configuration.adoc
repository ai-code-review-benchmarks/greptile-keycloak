...
Realm import and update Jobs inherit the scheduling of the server Pods if it is specified the Keycloak CR `spec.shceduling` field. If your server Pod scheduling is highly restrictive this may result in the 
Job Pods being unschedulable. In this situation you may override the Job scheduling using the Keycloak CR `spec.update.scheduling` and `spec.importScheduling` fields.
